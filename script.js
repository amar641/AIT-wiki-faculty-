...
function handleFacultyTypeSelection(facultyType) {
    ...
    botMessage(`Here are the ${facultyType} staff members:`);
    
    const selectedFaculty = faculty[facultyType];
    let facultyList = '';
    for (const name in selectedFaculty) {
        const details = selectedFaculty[name];
        facultyList += `<strong>${name}</strong><br>Email: ${details.email}<br>Phone: ${details.phone}<br><br>`;
    }
    
    botMessage(facultyList);
    
    // Restart the bot conversation
    botMessage("Would you like to search for more departments?");
...
function endChat() {
    userMessage("No");
    botMessage("Thank you for using the AIT Faculty Bot. Have a great day!");
